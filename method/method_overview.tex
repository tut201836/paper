...
LIPM では，重心高さを一定値 $z_{\mathrm{c}}$ と仮定し，角運動量の影響を無視することで，重心（Center of Mass: CoM）と ZMP の関係は次式で表される（$y$ 軸方向）．
\begin{equation}
...
この関係は，自然周波数
\begin{equation}
\omega := \sqrt{\frac{g}{z_{\mathrm{c}}}}
\label{eq:omega_def}
\end{equation}
を用いると，より明確に解釈できる．式 \eqref{eq:lipm_y} は
\begin{equation}
\ddot{y} = \omega^2 \left(y - y_{\mathrm{zmp}}\right)
\label{eq:lipm_y_omega}
\end{equation}
と書き換えられるため，$z_{\mathrm{c}}$ の増大は $\omega$ の低下に対応する．簡単のため $y_{\mathrm{zmp}}$ が短時間で一定であると仮定すると，$e := y - y_{\mathrm{zmp}}$ に対して
\begin{equation}
\ddot{e} = \omega^2 e
\label{eq:error_dyn}
\end{equation}
を得る．この系では，$\omega$ が大きいほど $e$ の増大が速くなり，横方向への倒れ込みが急激に進行する．したがって，重心高さを高くして $\omega$ を小さくすることは，転倒方向への発散を時間的に緩和する効果を持つと解釈できる．

以上より，LIPM の枠組みにおいては，重心高さ $z_{\mathrm{c}}$ の増大が横方向加速度の低下を通じて，転倒挙動を緩やかにする主要因であることが分かる．次に，この関係をより直感的かつ力学的に理解するため，支持脚足先を支点とした単純な振り子モデルを用いた考察を行う．
